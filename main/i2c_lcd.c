// // https://github.com/voidlooprobotech/ESP32_ESP-IDF_Code/tree/main/14_I2C_LCD_16x2
// #include "i2c_lcd.h"
// #include "esp_log.h"
...
esp_err_t err; // Variable to store I2C communication errors
static const char *TAG = "LCD"; // Tag for logging
static uint8_t backlight_state = LCD_BACKLIGHT_ON; // Track backlight state
...
void lcd_send_cmd(char cmd)
{
    ...
    data_t[0] = data_u | 0x0C | backlight_state; // Enable (EN) = 1, Register Select (RS) = 0
    data_t[1] = data_u | 0x08 | backlight_state; // Enable (EN) = 0, Register Select (RS) = 0
    data_t[2] = data_l | 0x0C | backlight_state; // Enable (EN) = 1, Register Select (RS) = 0
    data_t[3] = data_l | 0x08 | backlight_state; // Enable (EN) = 0, Register Select (RS) = 0
...
void lcd_send_data(char data)
{
    ...
    data_t[0] = data_u | 0x0D | backlight_state; // Enable (EN) = 1, Register Select (RS) = 1
    data_t[1] = data_u | 0x09 | backlight_state; // Enable (EN) = 0, Register Select (RS) = 1
    data_t[2] = data_l | 0x0D | backlight_state; // Enable (EN) = 1, Register Select (RS) = 1
    data_t[3] = data_l | 0x09 | backlight_state; // Enable (EN) = 0, Register Select (RS) = 1
...
void lcd_backlight_on(void)
{
    ...
    err = i2c_master_write_to_device(I2C_NUM, SLAVE_ADDRESS_LCD, &data, 1, 1000);
    if (err != 0) ESP_LOGI(TAG, "Error turning backlight on");
}
...
void lcd_backlight_off(void)
{
    ...
    uint8_t data = backlight_state;
    err = i2c_master_write_to_device(I2C_NUM, SLAVE_ADDRESS_LCD, &data, 1, 1000);
    if (err != 0) ESP_LOGI(TAG, "Error turning backlight off");
}